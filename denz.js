...
try {
		...
		const ownerNumberr = ["6285387059588@s.whatsapp.net","6285387059588@s.whatsapp.net",`${NomorOwner}@s.whatsapp.net`]
		...
		const isNsfw = isGroup ? anime.includes(from) : false
		...
		const katalog = (teks) => {
             res = denz.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© PoXKunZz_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             denz.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = denz.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'bit.ly/ownerpbotpoxcrafts', "groupName": `${NamaBot}`, "footerText": "*_© PoXKunZz_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
denz.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© PoXKunZz_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
...
reply(e)
			...
			if (budo.startsWith(`$`)){
                ...
                let exc = budo.replace(sep[0]+"\n", "")
                exec(exc, (err, stdout) => {
					if (err) return denz.sendMessage(from, `root @itzzpoxzenzz:~ ${err}`, text, { quoted: mek })
					if (stdout) {
        ...
        if (budy.includes("https://chat.whatsapp.com/")) {
        	if (!mek.key.fromMe){
				...
				reply('Bang ad yg ngemis member:v')
...
└─────────────────❒
${uwu}`
sendButLocation(from, `${menu}`, "*_© PoXKunZz_*", {jpegThumbnail:ofrply}, [{buttonId:`${prefix}command`,buttonText:{displayText:'LIST MENU'},type:1},{buttonId:`${prefix}owner`,buttonText:{displayText:'DEVELOPER'},type:1},{buttonId:`${prefix}script`,buttonText:{displayText:'SOURCE CODE'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
  /*buttons = [{buttonId:`${prefix}owner`,buttonText:{displayText:'OWNER'},type:1},{buttonId:`${prefix}script`,buttonText:{displayText:'SCRIPT'},type:1},{buttonId:`${prefix}command`,buttonText:{displayText:'MENU'},type:1}]
imageMsg = (await denz.prepareMessageMedia(fs.readFileSync('./denz.jpg'), 'imageMessage')).imageMessage
               buttonsMessage = {
               contentText: `${menu}`,
               footerText: `${uwu}P O X デンパ${uwu}`, 
               imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
await denz.sendMessage(from, buttonsMessage, MessageType.buttonsMessage, { thumbnail:ofrply, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title:`${jmn} - ${week} ${weton} - ${calender}`,body:"",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`bit.ly/ownerpbotpoxcrafts`}, mentionedJid: [ptod,dtod,otod,stod]}, quoted:finv})*/
break
case 'command':
 listMsg = {
 buttonText: 'LIST MENU',
 footerText: '*_© PoXKunZz_*',
 description: `Hai Kakk @${stod.split('@')[0]}, Silahkan pilih menu disini Jangan Lupa subs Owner ku ya kak><`,
 sections: [
                     {
...
denz.sendMessage(from, kon, image, { quoted: mek})
break
case 'lesbian':
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
reply(mess.wait)
...
denz.sendMessage(from, hai, image, { quoted: mek})
break
case 'wallpaper':
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
reply(mess.wait)
...
denz.sendMessage(from, buffer, image, { quoted: ftok})
break
case  'blowjob':
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
reply(mess.wait)
...
client.browserDescription = ['Pox Botz','Desktop','3.0']
...
break
		...
		case 'sc':
		case 'sourcecode':
		denz.sendMessage(from, { text: "https://github.com/ItzzPoxZenzz/PoxBotz", matchedText: 'https://github.com/ItzzPoxZenzz/PoxBotz', description: "", title: "don't click here !!!", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
       case 'debug':
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          ...
          "url": "https://github.com/ItzzPoxZenzz"
        },
        "index": 0
      }
    ]
...
▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒`, MessageType.extendedText,{
 quoted: {
  ...
  message: {
    ...
    status: 1,
    surface: 1,
    message: '☠️PoxSlayerZ☠️',
    orderTitle: 'PoxToXiCVirus', // Idk what this does
...
Giliran = @${tty.player1.split('@')[0]}`
 denz.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     denz.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Owner P - B0T",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./denz.jpg'),sourceUrl:"https://wa.me/6285387059588?text=Hi Kak Pox>///<"}}})
	}
    }
    }
    }
